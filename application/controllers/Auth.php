...
class Auth extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['url_google'] = $this->google->getUrl();
   ...
   public function login_google()
   {
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'nabilmuthi77@gmail.com',
            'smtp_pass' => 'ftemrxcepgdnlref',
            'smtp_port' => 465,
            'smtp_crypto' => 'ssl',
            'mailtype' => 'html',
            'charset' => 'utf-8',
        ...
        $this->email->from('nabilmuthi77@gmail.com', 'Nabilmuthi77');
        $this->email->to($this->input->post('email'));

        if ($type == 'verify') {
            $this->email->subject('Account Verification');
            $this->email->message('Click this link to verify your acount, You have <b>1 DAY</b> before token will be expired >> <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '"> Activate Now! </a>');
        } else if ($type == 'forgot') {
            $this->email->subject('Reset Password');
            $this->email->message('Click this link to reset your password, You have <b>1 DAY</b> before token will be expired >> <a href="' . base_url() . 'auth/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '"> Reset Password </a>');
        }
